...
# Step 1: Remove all aug images — training on originals only to cleanly
# measure the effect of having 33 good boards (ws+ns) vs 13 ws-only before
echo "=== Step 1: Remove aug images (training on originals only) ==="
python -c "
import glob, os
aug_files = glob.glob('data/ir_module_1024/ir_module/train/good/*_aug_*.jpg')
for f in aug_files:
    os.remove(f)
...
echo ""
echo "=== Step 2: Train PatchCore model4 (33 ws+ns boards, no aug, p=0.1) ==="
python bin/run_patchcore.py \
  --gpu 0 \
  --seed 0 \
  --save_patchcore_model \
  --log_group ir_module_WR50_L2-3_PS3_1024_m4_p0.1 \
  --log_project 640C \
  models/patchcore \
  patch_core \
    -b wideresnet50 \
...
echo ""
echo "=== Done. Model at models/patchcore/640C/ir_module_WR50_L2-3_PS3_1024_m4_p0.1/ ==="
